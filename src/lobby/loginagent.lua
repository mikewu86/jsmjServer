...
function REQUEST:loginReq()
	...
	if sign ~= usertoken then
		...
		if gameId == 0 then
			...
			skynet.call(dbmgr, "lua", "update_UserLastLobbyNode", self.uid, tonumber(skynet.getenv("NodeId")), skynet.getenv("nodename"))
		else
			LOG_DEBUG("login gamenode request")

			local serverNode = skynet.call(dbmgr, "lua", "find_GameServerNode_single", nodeId)
			if serverNode then
				dump(serverNode, "-----serverNode------")
				local secret = crypt.hexencode(crypt.randomkey())
				local nodeName = serverNode["NodeName"]
				LOG_DEBUG("findNode ok, nodename:"..nodeName)
				local queryok, remoteaddr = pcall(cluster.query, nodeName, "gated")
				if queryok then
					local cok, cresult = pcall(cluster.call, nodeName, remoteaddr, "login", self.uid, secret, skynet.getenv("nodename"))
					if cok then
						LOG_DEBUG("cluster.call node login ok, uid:"..self.uid)
						ret.serveraddr = serverNode["PublicHostIP"]..":"..serverNode["GatePort"]
				...
				LOG_ERROR("can not find node from db, nodeid:"..nodeId)
				ret.result = 0
				ret.message = "service not avaliable"
			end
		end
