...
function UserSingleEntity:load(uid)
	if not self.recordset[uid] then
		local record = skynet.call("dbmgr", "lua", "get_user_single", uid)
...
function UserSingleEntity:update(record, nosync)
	if not record.uid or not self.recordset[record.uid] then return end		-- 记录不存在，返回
	LOG_DEBUG("UserSingleEntity:update")
...
function UserSingleEntity:updateWinInfo(_record)
	local ret = skynet.call("dbmgr", "lua", "update_gameuser_game_win_lose", _record)
	--dump(ret, "res ret" )
	--dump(self.recordset, "recordset1")
	if ret then
		if not self.recordset[ret.value.Uid] then
			self.recordset[ret.value.Uid] = {}
			for k, v in pairs(ret.value) do 
				self.recordset[ret.value.Uid][k] = v
			end
		else
			self.recordset[ret.value.Uid][_record.key] = ret.value[_record.key]
		end
	end
	--dump(self.recordset, "recordset2")
end

function UserSingleEntity:updateMoney(record)
	if not record.userid or not self.recordset[record.userid] then
		LOG_ERROR("UserSingleEntity:updateMoney params error")
		return false
	end
	
	local doc = skynet.call("dbmgr", "lua", "update_gameuser_money",
        tonumber(record.userid),
        tonumber(record.wantedamount),
        tonumber(record.modifyamount),
        record.ModuleName,
        record.Remark,
        record.ipaddr)
	...
	if doc then
		for k, v in pairs(doc) do
			self.recordset[record.userid][k] = v
		end
		return true
...
function UserSingleEntity:get(uid, field)
	...
	if self.recordset[uid] then
		...
		--print("UserSingleEntity:get from cache")
		return record
	end
	...
	record = skynet.call("dbmgr", "lua", "get_user_single", uid) --不存在返回空的table {}
	...
	if table.empty(record) and type(orifield) == "string" then
		skynet.error("UserSingleEntity:get return null")
		return
	end

	print("UserSingleEntity:get from db")
