...
function CardAlgorithm.getBestGroupStraightFlush(_cards)
	...
	for _, g in pairs(groups) do
		if #g >= 5 then
			local gp = table.clone(g)
			CardAlgorithm.removeSameCard(gp)

			if #gp >= 5 then
				local hasA = false
				for _, card in pairs(gp) do
					if CardConstant.getCardValue(card) == CardConstant.VALUE_1 then
						hasA = true
						break
					end
				end
				...
				--如果牌里面有A还要按照A是最小牌值的方式搜索一次
				if hasA == true then
					CardAlgorithm.sortByValueReverse(gp, true)
					for i = 1, #gp - 4 do
						local continue5Cards = {}
						...
						if CardAlgorithm.isCardContinuousLess(continue5Cards, true) then
							table.insert(bestGroup, continue5Cards)
							find = true
							break
						end
...
function CardAlgorithm.getBestGroupFourKind(_cards)
	...
	if nil ~= groups then
		local cardCopy = table.clone(cards)
		--	找最大的一张
...
function CardAlgorithm.getBestGroupStraight(_cards)
	...
	if groupSize >= 5 then
		...
		for _, v in pairs(groups) do
			...
			if CardConstant.getCardValue(v[1]) == CardConstant.VALUE_1 then
				hasA = true
			end
		end
		...
		--如果牌里面有A还要按照A是最小牌值的方式搜索一次
		if hasA == true then
			CardAlgorithm.sortByValueReverse(singleCards, true)
			for i = 5, #singleCards do
				--	拷贝连续的5张牌
				local continue5Cards = {}
				for j = i - 4, i do
...
--牌按照从大到小排列，如果牌里面有A的话将返回两个结果，第一个为A当最大牌处理 第2个为A当最小牌处理
function CardAlgorithm.sortByValueReverse(_cards, aIsSmall)

	if not aIsSmall then
		table.sort(_cards, CardAlgorithm.sort_ValueLess)
	else
		table.sort(_cards, CardAlgorithm.sort_ValueLessAIsSmall)
	end
end
...
function CardAlgorithm.sort_ValueLessAIsSmall(_l, _r)
	return CardConstant.getCardValue(_l) > CardConstant.getCardValue(_r)
end

function CardAlgorithm.getCardRealValue(_card, aIsSmall)
...
function CardAlgorithm.isCardContinuousLess(_cards, aIsSmall)
	if #_cards == 1 then return true end
	...
	for i = 2, #_cards do
		local lv, rv
		if not aIsSmall then
			lv = CardAlgorithm.getCardRealValue(_cards[i - 1])
			rv = CardAlgorithm.getCardRealValue(_cards[i])
		else
			lv = CardConstant.getCardValue(_cards[i - 1])
			rv = CardConstant.getCardValue(_cards[i])
		end
