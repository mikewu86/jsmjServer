...
function GameEnd:calculateMoney(args)
    ...
    for k, countHuType in pairs(self.winnerCountHuTypeList) do
        ...
        if countHuType.isBaoPai then  -- 抢杠 一家支付
            if countHuType.dui3Zui and args.isZiMo then -- 吃碰杠某人3次
                totalMoney = totalMoney / 3 * 5
                if self.gameProgress.roomRuleMap['tc_extrarule1_100'] then
                    if totalMoney >= 100 then
                        totalMoney = 100
                    end
                elseif self.gameProgress.roomRuleMap['tc_extrarule1_200'] then
                    if totalMoney >= 200 then
                        totalMoney = 200
                    end
                end
                money[countHuType.pos] = money[countHuType.pos] + totalMoney
                money[countHuType.dui3ZuiPos] = money[countHuType.dui3ZuiPos] - totalMoney
            else
                if self.gameProgress.roomRuleMap['tc_extrarule1_100'] then
                    if totalMoney >= 100 then
                        totalMoney = 100
        ...
        elseif args.isZiMo == false then
            ...
            if self.gameProgress.roomRuleMap['tc_extrarule1_100'] then
                if perMoney >= 100 then
                    perMoney = 100
                    totalMoney = 300
                end
            elseif self.gameProgress.roomRuleMap['tc_extrarule1_200'] then
                if perMoney >= 200 then
                    perMoney = 200
                    totalMoney = 600
                end
            end
