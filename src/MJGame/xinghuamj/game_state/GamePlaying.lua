        ...
        for _, node in pairs(canTingNodes) do
            ...
            if tmpPlayer:delHandCard(removeCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    -- 5硬花
                    if self.gameProgress.huaLimit then
...
function GamePlaying:onUserCutBack(_pos, _uid)
    ...
    self.gameProgress:sendFengQuanInfo(_uid)
end
...
function GamePlaying:doGang(pos, byteCard, grabedPos)
    ...
    if true == ret then
        ...
        --             if i ~= pos then
        --                 tbMoney[i] = -money
        --                 if self.gameProgress.jScore > 0 and
        --                    self.gameProgress.playerList[i]:getLimitMoney() < money then
        --                     tbMoney[i] = -self.gameProgress.playerList[i]:getLimitMoney()
        --                 end
        --                 tbMoney[pos] = tbMoney[pos] - tbMoney[i]
        --             end
        --         end
        ...
        --     self.gameProgress:updatePlayersMoney(tbMoney, "立即结算风杠")
...
function GamePlaying:doHu(pos, byteCard)
    ...
    if not bZiMo then
        ...
        -- end
        if table.keyof(posList,self.gameProgress.banker) == nil then
            self.gameProgress.banker = self.gameProgress:nextPlayerPos(pos)
            self.gameProgress:addFengQuanCount()
        end
    else
        table.insert(posList, pos)
        if self.gameProgress.banker ~= pos then
            self.gameProgress.banker = self.gameProgress:nextPlayerPos(pos)
            self.gameProgress:addFengQuanCount()
        end
    end
