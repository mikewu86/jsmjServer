...
function GamePlaying:checkSelfHu(turn)
    ...
    if player:canSelfHu() then
        ...
        nil,
        false)
        local zhiCount = self:calcZhiFan(player)
        dump(zhiCount,"zhiCount ***")
        if zhiCount >= 8 then
            bHu = true
        end
        ...
        for _, node in pairs(canTingNodes) do
            ...
            if tmpPlayer:delHandCard(removeCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    local zhiCount = self:calcZhiFan(tmpPlayer)
                    if zhiCount >= 8 then
                        bHu = true
                    end
...
function GamePlaying:canHuOther(pos, turn, byteCard)
    ...
    if true == player:canHu(byteCard) then
        self.countHuType:setParams(
            ...
            turn,
            false)
        local zhiCount = self:calcZhiFan(player,byteCard)
        if zhiCount >= 8 then
            bHu = true
        end
        self.countHuType:calculate()
...
function GamePlaying:calcZhiFan(player,huCard)
    local wanHandNum = 0
    local tiaoHandNum = 0
    local tongHandNum = 0
...
function GamePlaying:doPeng(pos, byteCard, grabedPos)
    ...
    if true == ret then
        ...
        if self.gameProgress.roomRuleMap['wh_playrule_bj'] then
            self.gameProgress:checkAndBroadCastMsgWarningNotify(pos)
        end
