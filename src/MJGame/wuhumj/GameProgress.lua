...
local MJPlayer = require("WHMJPlayer")
...
local MJWarning = require("MJWarning")
-- 机器人出牌分析类
...
function GameProgress:ctor(room, roomId,unitCoin)
    ...
    self.roomRuleMap = {
       ['wh_playrule_sjf'] = false,    -- 胡牌三家付
       ['wh_playrule_bj'] = false,    -- 报警
       -- ['wh_playrule_sdzb'] = false, -- 四对转包
       -- ['wh_extrarule1_1234'] = false, -- 1234
       -- ['wh_extrarule1_2468'] = false, -- 2468
       ['wh_extrarule2_315'] = false, -- 50
       ['wh_extrarule2_525'] = false, -- 100
    }
    self:createGameRule()
    local baseScore = 30
    if self.roomRuleMap['wh_extrarule2_525'] then
        baseScore = 50
    end
    self.warningList = {}
    for i=1, self.maxPlayerCount do
        local player = MJPlayer.new(14, i, self.room.fangInfo.RoundCount, baseScore)
        player:setConstHuaList(self.huaList)
        local river = MJRiver.new()
        self.playerList[i] = player
        self.riverCardList[i] = river

        local warning = MJWarning.new(self, i)
        self.warningList[i] = warning
    end
...
function GameProgress:isVIPOver(isbegin)
    ...
    local ret = false
    if isbegin then
        if self.dRoundCount > self.room.fangInfo.RoundCount then
            ret = true
        end
    else
        if self.dRoundCount > self.room.fangInfo.RoundCount then
...
function GameProgress:addDRoundCount()
    local needAdd = false
    for _, player in pairs(self.playerList) do 
        local difen = player:getOneDiFen(self.dRoundCount)
        if difen <= 0 then
            needAdd = true
        end
    end
    if needAdd then
        self.dRoundCount = self.dRoundCount + 1
    end
end
...
function GameProgress:sendTotalScore(_uid)
    -- body
    local score = ""
    -- if self.room.fangInfo.RoundCount == 1 then
    --     for i=1,4 do
    --         if i == 4 then
    --             score = score..self.playerList[i].diFenList[1]
    --         else
    --             score = score..self.playerList[i].diFenList[1].."#"
    --         end
    --     end
    ...
    --     for i=1,4 do
    --         if i == 4 then
    --             score = score..self.playerList[i].diFenList[1].."/"..self.playerList[i].diFenList[2]
    --         else
    --             score = score..self.playerList[i].diFenList[1].."/"..self.playerList[i].diFenList[2].."#"
    --         end
    --     end 
    ...
    for pos = 1, 4 do
        for i=1,self.room.fangInfo.RoundCount do
            score = score..self.playerList[pos].diFenList[i]
            if i ~= self.room.fangInfo.RoundCount then
                score = score.."/"
            end
        end
        if pos ~= 4 then
    ...
    local jiaoTb = {}
    for i=1,4 do
        table.insert(jiaoTb,self.playerList[i].jiaoFen)
    end
    pkg.userScore = score
    pkg.userJiao = jiaoTb
...
--- 判断某张牌出的数量
function GameProgress:CheckCardOutNum(_byteCard)
    --LOG_DEBUG("_byteCard: ".._byteCard)
    if MJConst.kMJPointNull == _byteCard then --- 如果 值非法，则返回大于4的值
        return 5
    end
    --- 从牌河 和 pile中判断
    local riverCards = self.riverCardList
    local cardNumInRiver = 0
    for _, cardRiver in pairs(riverCards) do
        cardNumInRiver = cardNumInRiver + cardRiver:getCardCount(_byteCard)
    end
    local cardNumInPiles = 0
    for _, player in pairs(self.playerList) do 
        cardNumInPiles = cardNumInPiles + player:getCardNumInPiles(_byteCard)
    end
    --LOG_DEBUG("_byteCard: ".._byteCard.." out: "..cardNumInRiver.." piles:"..cardNumInPiles)
    return (cardNumInRiver + cardNumInPiles)
end

function GameProgress:checkAndBroadCastMsgWarningNotify(_pos)
    for pos, warning in pairs(self.warningList) do 
        if nil ~= warning and pos == _pos then
            local ret,tb = warning:checkWaring()
            if true == ret then
                self:warningNotifyPkgHandle(tb, pos)
            end
        end
    end
...
function GameProgress:warningNotifyPkgHandle(_nAli, _myPos)
    local pkg = {}
    pkg.Type = _nAli
    pkg.MyPos = _myPos
    self.operHistory:addOperHistory('warningNotify', pkg)
    self.room:broadcastMsg("warningNotify", pkg)
end
