...
local MJPlayer = require("ZJMJPlayer")
...
function GameProgress:sendMsgToPlayerClientNotifyZhengDa(_pos, _type, _args)
    local pkg = {}
    pkg.OperationSeq = self:getOperationSeq()
    pkg.NotifyType = _type
    pkg.Params = _args
    self.room:sendMsgToSeat(_pos, "playerClientNotify", pkg)
end
