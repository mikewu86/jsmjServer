...
-- 及时结算花数
local kAGHua = 2 -- 两倍底钱
local kMGHua = 1 -- 面下杠也按照明杠计算，一倍底钱
    ...
    if player then
        if false == player:hasNewCard() then
            ...
            if player:hasBaiDa(self.gameProgress.baiDa) ~= 0 then
                if true == player:canSelfHu(self.gameProgress.baiDa, true) or true == player:canSelfHuQiDui(self.gameProgress.baiDa, true) then
                    LOG_DEBUG("PLAYER can hu self with TUODA")
                    bHu = true
                    self.isTuoDa = true
                elseif true == player:canSelfHu(self.gameProgress.baiDa, false) or true == player:canSelfHuQiDui(self.gameProgress.baiDa, false) then
                    LOG_DEBUG("PLAYER can hu self with DA")
                    bHu = true
                    self.isTuoDa = false
                end
            else
                -- 无搭情况下
                if true == player:canSelfHu(self.gameProgress.baiDa, true) or true == player:canSelfHuQiDui(self.gameProgress.baiDa, true) then
                    LOG_DEBUG("PLAYER can hu self with no DA")
                    bHu = true
                    self.isTuoDa = false
                end
            end
...
function GamePlaying:checkOtherOper(turn, byteCard, _bGrabGang)
    ...
    for i = 1, self.gameProgress.maxPlayerCount - 1 do  -- 仅允许除出牌人以外的人抢牌
        ...
        if not self.mjCancelMgr:isCancelOper(pos, MJConst.kOperHu, byteCard) then
            -- 胡别人的牌，只能脱搭，但不算脱搭
            if true == player:canHu(byteCard, self.gameProgress.baiDa, true) or true == player:canHuQiDui(byteCard, self.gameProgress.baiDa, true) then
                LOG_DEBUG("can hu")
                self.countHuType:setParams(
                    pos, 
                    self.gameProgress, 
...
function GamePlaying:onUserCutBack(_pos)
    ...
    -- 发送正搭牌
    self.gameProgress:sendMsgToPlayerClientNotifyZhengDa(
        _pos,
        kZhengDa,
        -- 转换牌值后发送
...
function GamePlaying:onPlayCard(_pos, _pkg)
    ...
    -- 判断是首搭，中途打搭，还是碰后打搭
    -- 首搭
    if self.riverCardList[pos]:getCount() == 0 and byteCard == self.gameProgress.baiDa then
        self.gameProgress.playDa[pos][1] = 1
        LOG_DEBUG("POS "..pos.." play SHOU DA")
        dump(self.gameProgress.playDa)
    -- 中途打搭，每打一张加1计数
    elseif self.riverCardList[pos]:getCount() ~= 0 and byteCard == self.gameProgress.baiDa then
        self.gameProgress.playDa[pos][2] = self.gameProgress.playDa[pos][2] + 1
        LOG_DEBUG("POS "..pos.." play DA")
        dump(self.gameProgress.playDa)
    end

    -- 碰后打搭
    -- 上一次操作是否是碰，每打一张加1计数
    if player.opHistoryList[#player.opHistoryList - 1] == MJConst.kOperPeng and byteCard == self.gameProgress.baiDa then
        self.gameProgress.playDa[pos][3] = self.gameProgress.playDa[pos][3] + 1
        LOG_DEBUG("POS "..pos.." play DA after PENG")
        dump(self.gameProgress.playDa)
    end
...
function GamePlaying:giveNewCard(turn)
    ...
    -- 拿牌之前检测是不是跑搭
    if player:checkPaoDa(self.gameProgress.baiDa) then
        self.isPaoDa = true
    else
        self.isPaoDa = false
    end
...
function GamePlaying:doPeng(pos, byteCard, grabedPos)
    ...
    if true == ret then
        ...
        self:checkMyOper(pos, false)  -- 仅让玩家
    end
    return ret
end
...
function GamePlaying:doHu(pos, byteCard)
    ...
    isTuoDa = self.isTuoDa,
    isPaoDa = self.isPaoDa })
end
