...
--  AKMJPlayer
local AKMJPlayer = class("AKMJPlayer", BasePlayer)

function AKMJPlayer:ctor(maxCardCount, pos, isDiaoDa)
...
function AKMJPlayer:setConstHuaList(huaList)
    self.constHuaList = huaList
end

function AKMJPlayer:clear()
    self.super.clear(self)
end

function AKMJPlayer:reset()
    self:clear()
    self.huaList = {}
    self.huaGangList = {}
end
...
function AKMJPlayer:getHuaList()
    return self.huaList
end

function AKMJPlayer:getCardsForNums(_step)
...
-- 某种牌在明牌中数量去掉百搭
function AKMJPlayer:getSuitCountInHandWithBaida(suit, bIncludeNewCard,baida)
    local sum = 0
...
function AKMJPlayer:getCardNumInHua(_cardByte)
    local sum = 0
...
function AKMJPlayer:isHuaGang(_cardByte)
    -- local bRet = false
...
function AKMJPlayer:getHuaGangNum()
    local huaGangSum = #self.huaGangList
    return huaGangSum
end

-- 手牌中有没有百搭牌，有的话返回数量，没有返回0
function AKMJPlayer:hasBaiDa(_baiDa)
    local baiDaNum = 0
...
-- 生成一个新对象
function AKMJPlayer:clone()
    local player = AKMJPlayer.new(self.maxCardCount, self.myPos)
...
-- 获取所有可以听的牌
function AKMJPlayer:getCanTingCards(baiDaList)
...
-- 获取可以听以及可以出的牌
function AKMJPlayer:getTingNormalCards(baiDaList)
    local ret = {}
...
-- 可以听七对的牌
function AKMJPlayer:getTingQiDuiCards(baiDaList)
    local ret = {}
