        ...
        for _, node in pairs(canTingNodes) do
            ...
            if tmpPlayer:delHandCard(removeCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    table.insert(_node.huList, {card = canHudCard, fan = 2, left = _left })
                end
    ...
    if player then
        if false == player:hasNewCard() then
            ...
            if true == checkOp then
                -- 胡牌
                if player:canSelfHu({self.gameProgress.baiDa}) or 
                player:getCardCountInHand(MJConst.Zi5, true) == 4 or 
                ( self.gameProgress.roomRuleMap['ak_playrule_7dui'] and 
                    player:canSelfHuQiDui({self.gameProgress.baiDa}))
                 then
                    self.countHuType:setParams(
                    turn, 
                    self.gameProgress, 
                ...
                local baidaCount = player:hasBaiDa()
...
function GamePlaying:canHuOther(pos, turn, byteCard,baiDa)
    ...
    if true == player:canHu(byteCard,{baiDa}) or 
         player:canHuQiDui(byteCard,{baiDa}) then
        self.countHuType:setParams(
            pos, 
            self.gameProgress, 
            byteCard, 
...
function GamePlaying:checkOtherOper(turn, byteCard, _bGrabGang)
    ...
    for i = 1, self.gameProgress.maxPlayerCount - 1 do  -- 仅允许除出牌人以外的人抢牌
        ...
        --if not self.mjCancelMgr:isCancelOper(pos, MJConst.kOperHu, nil) then
            if true == player:canHu(byteCard, {self.gameProgress.baiDa}) and
             true == _bGrabGang then
                -- self.countHuType:setParams(
                --     pos, 
                --     self.gameProgress, 
                --     byteCard, 
        ...
        if false == _bGrabGang then
            ...
            --     local chiTypeList = player:getCanChiType(byteCard,{self.gameProgress.baiDa})
            --     if (handCount - 2) >  baidaCount then
...
function GamePlaying:giveNewCard(turn)
    ...
    if nil == byteCard then
        local RemainCardNum = self.mjWall:getRemainCardsNum()
        local ZhaMaCount = 0
        if self.gameProgress.roomRuleMap['ak_extrarule1_1m'] then
            ZhaMaCount = 1
        elseif self.gameProgress.roomRuleMap['ak_extrarule2_6m'] then
            ZhaMaCount = 6
        elseif self.gameProgress.roomRuleMap['ak_extrarule2_4m'] then
            ZhaMaCount = 4
        end
        if RemainCardNum > ZhaMaCount then
...
function GamePlaying:doHu(pos, byteCard)
    ...
    local fans = self.gameProgress:calcExtraFans(posList, cards)
    self.gameProgress:broadCastExtraAwardCardsEx(posList, cards, 0 )
