...
function GamePlaying:checkSelfHu(turn,baiDa)
    ...
    if player:canSelfHu({baiDa}) or player:canSelfHuQiDui({baiDa}) then
        bHu = true
    end
    return bHu
end
        ...
        if false == player:hasNewCard() then
            ...
            if true == checkOp then
                ...
                if player:canSelfHu({self.gameProgress.baiDa}) or 
                    (self.gameProgress.roomRuleMap['sz_playrule_7dui'] and player:canSelfHuQiDui({self.gameProgress.baiDa})) then
                    bHu = true
                    if bHu then
...
function GamePlaying:canHuOther(pos, turn, byteCard, baiDa)
    ...
    if true == player:canHu(byteCard,{baiDa}) or true == player:canHuQiDui(byteCard,{baiDa}) then
        bHu = true
...
function GamePlaying:checkOtherOper(turn, byteCard, _bGrabGang)
    ...
    for i = 1, self.gameProgress.maxPlayerCount - 1 do  -- 仅允许除出牌人以外的人抢牌
        ...
        if not self.mjCancelMgr:isCancelOper(pos, MJConst.kOperHu, nil) then
            if true == player:canHu(byteCard, {self.gameProgress.baiDa}) or  
                (self.gameProgress.roomRuleMap['sz_playrule_7dui'] and player:canHuQiDui(byteCard,{self.gameProgress.baiDa})) then
                local bHu = true
