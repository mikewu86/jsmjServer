...
local MJPlayer = require("SZMJPlayer")
...
function GameProgress:ctor(room, roomId,unitCoin)
    ...
    self.huaList = {}
    for i = MJConst.Zi5, MJConst.Blank do
    ...
    self.roomRuleMap = {
        ['sz_playrule_7dui']   = false, -- 胡七对  
        ['sz_playrule_yzm']   = false, -- 硬自摸   
        ['sz_extrarule1_bufan'] = false, -- 不翻牌 
        ['sz_extrarule1_2z'] = false, -- 翻2张    
        ['sz_extrarule1_4z'] = false, -- 翻4张    
        ['sz_extrarule1_6z'] = false, -- 翻6张
    }
...
function GameProgress:getZhaMaCount(cards)
    ...
    for k, v in pairs(cards) do
        ...
        if card.suit <= MJConst.kMJSuitTong and card.suit >= MJConst.kMJSuitWan then
            if card.point == MJConst.kMJPoint1 or card.point == MJConst.kMJPoint5 or card.point == MJConst.kMJPoint9 then
                count = count + 1
            end
        elseif card.suit == MJConst.kMJSuitZi then
            if card.point >= MJConst.kMJPoint1 and card.point <= MJConst.kMJPoint4 then
                count = count + 1
            end
        elseif card.suit == MJConst.kMJSuitBlank and card.point == MJConst.kMJPoint2 then
            count = count + 1
        end
    end
    dump(count,"count ***")
...
function GameProgress:getNeedCardsAndBroadcast()
    local needCardNum = 0
    if self.roomRuleMap.sz_extrarule1_2z == true then
        needCardNum = 2
    elseif self.roomRuleMap.sz_extrarule1_4z == true then
        needCardNum = 4
   elseif self.roomRuleMap.sz_extrarule1_6z == true then
        needCardNum = 6
    end
    local cards = {}
