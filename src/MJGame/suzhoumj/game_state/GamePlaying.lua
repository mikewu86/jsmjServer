        ...
        for _, node in pairs(canTingNodes) do
            ...
            if tmpPlayer:delHandCard(removeCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    if bHu then
                        local cSongGang = self:canSongGang(player)
                        if not cSongGang then
                            bHu = false
                        end
                    end
            ...
            if true == checkOp then
                ...
                if player:canSelfHu() or player:canSelfHuQiDui() then
                    ...
                    if bHu then
                        ...
                        if not cSongGang then
                            bHu = false
                            self.mjCancelMgr:addCancelOper(turn, MJConst.kOperHu, player.newCard)
                        end
                    end
...
-- 送杠
function GamePlaying:canSongGang(player)
    local len = #player.opHistoryList
    if len > 1 then
        local justDoOper = 0
        for i=1,#player.opHistoryList do
            justDoOper = player.opHistoryList[len - i]
            if justDoOper ~= MJConst.kOperBuHua then
                break
            end
        end
        if justDoOper == MJConst.kOperMG then
            local pos = player.pileList[#player.pileList].from

            local isMenQing = self:isMengQing(player)
...
function GamePlaying:isMengQing(player)
    local hualist = player:getHuaList()
    local pilelist = table.copy(player.pileList)
    local card = pilelist[#pilelist].cardList[1]

    if #pilelist > 0 then
        table.remove(pilelist, #pilelist)
    end

    -- 软化 风暗刻 暗杠 风暗杠
    -- local anKe = self:getFengKeCount()
    -- local anGang = self:getAGangCount()
    -- local anFGang = self:getFengAGangCount()
    ...
    if card and cpyMap[card] then
        cpyMap[card] = cpyMap[card] + 3
    end

    for startI = MJConst.Zi1, MJConst.Zi4 do
        if cpyMap[startI] == 3 then
            fCount = fCount + 1
        end
    end
    ...
    if count == 0 then
        ...
        if #pilelist > 0 then
            for k, pile in pairs(pilelist) do
