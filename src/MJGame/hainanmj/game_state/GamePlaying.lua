...
function GamePlaying:ctor(gameProgress, stateId)
    ...
    self.hasFan = {false, false, false, false}  -- 是否有番
...
function GamePlaying:getCardShowedCount(byteCard)
    ...
    for _, player in pairs(self.playerList) do
        cnt = cnt + player:getCardCountInPileWithoutAG(byteCard)
    end
        ...
        for _, node in pairs(canTingNodes) do
            ...
            if tmpPlayer:delHandCard(node.playCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    table.insert(_node.huList, {card = canHudCard, fan = 0, left = 0 })
                end
            end
...
function GamePlaying:checkSelfHu(turn)
    ...
    if player:canSelfHu() == true or 
        player:canSelfHuQiDui() == true or
        player:canSelfHu13Yao() == true then
        bHu = true
    end
    return bHu
end
...
function GamePlaying:checkMyOper(turn, checkOp)
    local player = self.playerList[turn]
    self.hasFan = {false, false, false, false}
    if player then
        ...
        if false == player:hasNewCard() then
            ...
            local cancel = false
            local bHu = false
            if checkOp == true then 
                -- 胡牌
                if player:canSelfHu() == true or 
                    player:canSelfHuQiDui() == true or
                    player:canSelfHu13Yao() then
                    bHu = true
                end
                if bHu then
            ...
            if #canTingNodes == 0 then
                canTingNodes = player:getTing13YaoNodes()
            end
            -- dump(canTingNodes, ' canTingNodes = ', 6)
            local tipList = nil
            if #canTingNodes > 0 then
...
local bHu = false
    ...
    if player:canHu(byteCard) == true or 
        player:canHuQiDui(byteCard) == true or
        player:canHu13Yao(byteCard) == true then
        self.countHuType:setParams(
            ...
            self.isQiangGang)
        self.countHuType:calculate()
        if self.gameProgress.isYouFan then
            ...
            if player:canHu(byteCard) == true or 
                ...
                if self.gameProgress.isYouFan then
                    for k, huType in pairs(HuTypeConst.kCanHuTypeList) do
                        if self.countHuType:hasHuType(huType) then
                            bHu = true
                            self.hasFan[pos] = true
                            -- LOG_DEBUG('huType = '..huType)
                            break
                        end
                    end
                    if bHu == false then
                        if _bGrabGang == true then
                            bHu = true
                        end
                    end
                else
...
function GamePlaying:onUserCutBack(_pos, _uid)
    ...
    -- self.gameProgress:broadcastCutUserList()
    dump(self.gameProgress.gaList,"self.gameProgress.gaList playing")
    -- 发送嘎分
...
function GamePlaying:onGrabCard(_pos, _pkg)
    ...
    if canDoMap and canDoMap[oper] then 
        ...
        if self:notNeedWait(oper, pos) == true then
            -- LOG_DEBUG("exec op.")
            self:grabEnd()
        else
...
--- 复写 一炮多响的等待判断,海南麻将截胡。
function GamePlaying:notNeedWait(oper, pos)
    local bPowerestWait = self.playerGrabMgr:needWait()
    local needWaitHuMore = false
    local bRet = false
    if MJConst.kOperHu == oper then
        needWaitHuMore = true
        -- 查找胡操作玩家 的数量
        if self.multiHuPos[1] == pos then
            needWaitHuMore = false
        else
            local canHuNum = 0
    ...
    bRet = (bPowerestWait == false) and (needWaitHuMore == false)
    return bRet
end
...
function GamePlaying:onPlayCard(_pos, _pkg)
    ...
    for k, v in pairs(self.playerList) do
        if #v.playedList == 1 and
           v.playedList[1] == byteCard  then
        else
            isGeng4 = false
            break
        end
    ...
    if isGeng4 then
        self.gameProgress:broadcastBaoPaiType(self.gameProgress.banker, 1)
    end
    -- 判断其他玩家是否可以操作
...
function GamePlaying:addBaoPai(pos, oper)
    local baoOperList = table.copy(MJConst.chiList)
    table.insert(baoOperList, MJConst.kOperPeng)
    table.insert(baoOperList, MJConst.kOperMG)
    local baoInfo = nil
    if table.keyof(baoOperList, oper) ~= nil then
        if self.playerList[pos] then
            baoInfo = self.playerList[pos]:getBaoPaiInfo()
            if baoInfo.type == 3 or baoInfo.type == 4 then
                self.gameProgress:addBaoPai(baoInfo.type,
                baoInfo.pos, pos)
            end
        end
    end
...
function GamePlaying:grabEnd()
    ...
    if grabNode == nil then   -- 没人抢牌，继续发牌
        ...
        elseif table.keyof(MJConst.chiList, oper) ~= nil  then
...
function GamePlaying:buHua(turn)
    ...
    if  true == player:hasHua() then
        ...
        -- -- 判断是否为花杠
        -- local huaHuType  = self.gameProgress:getHuaHuType(turn)
        -- -- dump(bHuaGang,' is huaGang ')
        -- if huaHuType > 0 then
        --     local money = self:getGangScore(turn, huaHuType == 2)
        --     self.gameProgress:addRoundHuaHuScore(money)
        -- end

        local huaCardList = player:getHuaList()
        --发送增加花牌数消息
        self.gameProgress:broadcastFlowerCardCountNotify(
            player.myPos, #huaCardList, huaByteCard
        )
...
function GamePlaying:giveNewCard(turn)
    -- 流局控制
    if self.mjWall:getCanGetCount() == 15 then
        self:gotoNextState({isLiuJu = true})
        return
    end
    local player = self.playerList[turn]
    ...
    player:addNewCardList(byteCard) -- 把新牌加入列表
...
function GamePlaying:doPeng(pos, byteCard, grabedPos)
    ...
    if true == ret then
        local baoInfo = self:addBaoPai(pos, MJConst.kOperPeng)
        local baoType = 0
        if baoInfo then
            baoType = baoInfo.type
        end
        ...
        self.gameProgress:broadcastMsgAddPengCards(seq, pos, grabedPos, byteCard, baoType)
...
function GamePlaying:doGang(pos, byteCard, grabedPos)
    ...
    if true == ret then
        local baoInfo = self:addBaoPai(pos, MJConst.kOperMG)
        local baoType = 0
        if baoInfo then
            baoType = baoInfo.type
        end
        ...
        -- 发送消息
        self.gameProgress:opHandCardNotify(seq, pos, lCards, kRemoveCards)
        self.gameProgress:broadcastMsgAddGangCards(seq, pos, grabedPos, byteCard, baoType)
        self.riverCardList[grabedPos]:popCard()
        self.gameProgress:broadCastMsgOpOnDeskOutCard(seq, kRemoveCards, grabedPos, byteCard)
...
function GamePlaying:doChi(pos, chiType, byteCard, grabedPos)
    ...
    if ret then
        local baoInfo = self:addBaoPai(pos, chiType)
        local baoType = 0
        if baoInfo then
            baoType = baoInfo.type
        end
        ...
        -- 发送消息
        self.gameProgress:opHandCardNotify(seq, pos, lCards, kRemoveCards)
        self.gameProgress:broadcastMsgAddChiCards(seq, pos, grabedPos, sendCards, baoType)
        self.riverCardList[grabedPos]:popCard()
        self.gameProgress:broadCastMsgOpOnDeskOutCard(seq, kRemoveCards, grabedPos, byteCard)
...
function GamePlaying:doMXGang(pos, byteCard, grabedPos)
    ...
    if true == ret then
        ...
        self.gameProgress:opHandCardNotify(seq, pos, lCards, kRemoveCards)
        self.gameProgress:broadcastMsgAddGangCards(seq, pos, fromPos, byteCard, 0)
        -- 检是否有人抢杠
...
function GamePlaying:doAnGang(pos, byteCard, grabedPos)
    ...
    if true == ret then
        ...
        -- 发送消息 增加面下杠不需要再发送删除之前碰牌数据，客户端自行处理
        self.gameProgress:opHandCardNotify(seq, pos, lCards, kRemoveCards)
        self.gameProgress:broadcastMsgAddGangCards(seq, pos, grabedPos, byteCard, 0)
        self:giveNewCard(pos)
    end
    return ret
end
...
function GamePlaying:doHu(pos, byteCard)
    ...
    local posList = {}
    if not bZiMo then  -- 海南麻将需要谁点过胡,找出离出牌人最近的那个人
        posList = {self.multiHuPos[1]}
        for k, v in pairs(self.multiHuPos) do
            if self.playerGrabMgr:isDoOper(v, MJConst.kOperHu) then
                posList = {v}
                break
            end
        end
