    ...
    if player then
        if false == player:hasNewCard() then
            ...
            if true == checkOp then
                ...
                ( self.gameProgress.roomRuleMap['hz_playrule_7dui'] and 
...
function GamePlaying:doHu(pos, byteCard)
    ...
    if not bZiMo then
        ...
        if #posList > 1 then
            self.gameProgress.banker = self.gameProgress:nextPlayerPos(self.playerGrabMgr.pos)
        else
            self.gameProgress.banker = self.gameProgress:nextPlayerPos(pos)
        end
    else
