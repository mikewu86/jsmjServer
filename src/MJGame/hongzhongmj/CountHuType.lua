...
function CountHuType:calculate()
    ...
    if self.isZiMo then
        ...
        if self.isHu then
            huDetails.describe = huDetails.describe..'自摸6分;'
            hua = hua + 6
        else
            huDetails.describe = huDetails.describe..'自摸-2分;'
            hua = hua - 2
        end
    end
    ...
    if self.isQiangGang then
        if self.isHu then
            huDetails.describe = huDetails.describe..'抢杠6分;'
            hua = hua + 6
        elseif self.pos == self.fangPaoPos then
            local HuFans = 6 * self.HuCount
    ...
    if mGangCount > 0 then
        ...
        mJieGang = mJieGang + mGangCount * 6
    end
    ...
    if pGangCount > 0 then
        ...
        mJieGang = mJieGang + pGangCount * 6
    end
    ...
    if aGangCount > 0 then
        ...
        mJieGang = mJieGang + aGangCount * 9
    end
    ...
    if mDainCount > 0 then
        mDianGang = mDianGang + mDainCount * 6
    end

    -- 被暗杠
    local mBAnGangCount = self:getOtherAGangCount()
    if mBAnGangCount > 0 then
        mDianGang = mDianGang + mBAnGangCount * 3
    end
    ...
    if mBPGangCount > 0 then
        mDianGang = mDianGang + mBPGangCount * 2
    end
    ...
    if self.isLiuJu ~= true then
        local ZhaMa = self.gameProgress.ZhaMaFen
        if ZhaMa > 0 then
            if self.isHu then
                ZhaMa = ZhaMa * 3
                huDetails.describe = huDetails.describe..'扎码'..ZhaMa..'分;'
                hua = hua + ZhaMa
            else
                if self.isQiangGang then
                    if self.fangPaoPos == self.pos then
                        ZhaMa = ZhaMa * 3
                        local zmFans = ZhaMa * self.HuCount    
                        huDetails.describe = huDetails.describe..'扎码-'..zmFans..'分;'
                        hua = hua - zmFans
                    end
                else
                    huDetails.describe = huDetails.describe..'扎码-'..ZhaMa..'分;'
