...
local MJPlayer = require("HZMJPlayer")
...
function GameProgress:ctor(room, roomId,unitCoin)
    ...
    self.roomRuleMap = {
       -- ['ks_playrule_baozi'] = false,    -- 豹子
       -- ['ks_extrarule1_4z'] = false,    -- 四百搭
       -- ['ks_extrarule1_3z'] = false, -- 三百搭
        ['hz_playrule_7dui'] = false,  -- 可胡七对
        ['hz_playrule_1m'] = false,    -- 一码全中
        ['hz_playrule_z2m'] = false, -- 扎2码
        ['hz_playrule_z4m'] = false, -- 扎4码
        ['hz_playrule_z6m'] = false, -- 扎6码
    }
...
function GameProgress:getZhaMaCount(cards)
    self.ZhaMaIndex = {}
    local num = 0
    local count = 0
    for k, v in pairs(cards) do
        local value = v % MJConst.kMJPointNull
        if self.roomRuleMap.hz_playrule_1m == true then
            ...
            count = 1
            table.insert(self.ZhaMaIndex, k)
        else
            if value == 1 or
            value == 5 or       -- 红中也是5
            value == 9 then
    ...
    if self.roomRuleMap.hz_playrule_1m == false then
        num = num * 2
    end

    self.ZhaMaFen = num
    self.ZhaMaCount = count
    return num
end
...
function GameProgress:getNeedCardsAndBroadcast()
    ...
    if self.roomRuleMap.hz_playrule_1m == true then
        needCardNum = 1
    elseif self.roomRuleMap.hz_playrule_z2m == true then
        needCardNum = 2
    elseif self.roomRuleMap.hz_playrule_z4m == true then
        needCardNum = 4
   elseif self.roomRuleMap.hz_playrule_z6m == true then
        needCardNum = 6
    end
    local cards = {}
...
function GameProgress:broadCastExtraAwardCardsEx(winPos, cards, type)
    ...
    pkg.type = type
    pkg.b1mqz = 0
    pkg.fen = self.ZhaMaFen
    pkg.count = self.ZhaMaCount
    pkg.indexlist = self.ZhaMaIndex
...
function GameProgress:calcExtraFans(winPos, cards)
    ...
    for _, pos in pairs(winPos) do 
        fans[pos] = self:getZhaMaCount(cards)
    end
