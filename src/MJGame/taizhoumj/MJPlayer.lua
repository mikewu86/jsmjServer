...
-- 把手牌转成牌的数量Map
function MJPlayer:transHandCardsToCountMap(bIncludeNewCard,filterCards)
    local map = {}
    ...
    if filterCards and #filterCards > 0 then
        for k,v in pairs(filterCards) do
            if v then
                if map[v] > 0 then
                    map[v] = map[v] - 1
                end
...
-- 能否杠
function MJPlayer:canGang(byteCard, ting)
    if self:hasNewCard() then
        return false
    end
    local cards = self:getAllHandCards()
    table.insert(cards, byteCard)
    local cardmap = self:transferCardsToMap(cards)
    if #self.cardList > 6 then
        if self:getCardCountInHand(byteCard) >= 3 then
            if ting == true then
                cardmap[byteCard] = cardmap[byteCard] - 3
                local tingNode = self:checkTingCard(cardmap, byteCard)
                cardmap[byteCard] = cardmap[byteCard] + 3
                if tingNode ~= nil then
                    return true                     
                end
            else
                return true
...
-- 查询可以暗杠的牌
function MJPlayer:getAnGangCardList(ting)
    local list  ={}
    if not self:hasNewCard() then
        return list
    end
    ...
    if #self.cardList > 6 then
        for k, v in pairs(self.cardList) do
            if nil == table.keyof(list, v) then
                ...
                if sum == 4 then
                    if ting == true then
                        cardmap[v] = cardmap[v] - 3
                        local tingNode = self:checkTingCard(cardmap, v)
                        cardmap[v] = cardmap[v] + 3
                        if tingNode ~= nil then
                            table.insert(list, v)                        
                        end
...
-- 获取所有可以听的牌
function MJPlayer:getCanTingCards(filterCards)
...
-- 获取可以听以及可以出的牌
function MJPlayer:getTingNormalCards(filterCards)
    local ret = {}
    if not self:hasNewCard() then
        return ret
    end
    ...
    for byteCard, count in pairs(countMap) do
        local tingNode = self:checkTingCard(cpyMap, byteCard)
        if tingNode ~= nil then
...
function MJPlayer:isRightCard(huCard)
    if huCard == nil then
        return false
    end
    local card = MJCard.new({byte = huCard})
    if card.suit == MJConst.kMJSuitHua then
        return false
    end
    if card.suit == MJConst.kMJSuitZi then
        return false
    end
    if card.point == MJConst.kMJPoint1 or card.point == MJConst.kMJPoint2 or
...
function MJPlayer:transferCardsToMap(cards)
    local map = {}
    for i=0, MJConst.Zi7 do
        map[i] = 0
    end
...
function MJPlayer:checkTingCard(cardMap, playCard)
    local tingNode = nil
    local lcardMap = cardMap
    if playCard > 0 and lcardMap[playCard] > 0 then
        lcardMap[0] = lcardMap[0]+1                  -- 增加一张百搭
        lcardMap[playCard] = lcardMap[playCard] - 1  -- 去掉当前手牌
        if self.mjMath:canHu(lcardMap) then
            lcardMap[0] = lcardMap[0] - 1
            local huCards = self.mjMath:getCanHuCards(lcardMap)
            if #huCards == 1 then
                local huCard = huCards[1]
                if self:isRightCard(huCard) then
                    if lcardMap[huCard + 1] > 0 and lcardMap[huCard - 1] > 0 then
                        lcardMap[huCard + 1] = lcardMap[huCard + 1] - 1
                        lcardMap[huCard - 1] = lcardMap[huCard - 1] - 1
                        if self.mjMath:canHu(lcardMap) == true then
                            tingNode = TingNode.new(playCard, huCards)
                        end
                        lcardMap[huCard + 1] = lcardMap[huCard + 1] + 1
                        lcardMap[huCard - 1] = lcardMap[huCard - 1] + 1   
                    end
            ...
            lcardMap[playCard] = lcardMap[playCard] + 1
        else
            lcardMap[0] = lcardMap[0] - 1
            lcardMap[playCard] = lcardMap[playCard] + 1
        end
