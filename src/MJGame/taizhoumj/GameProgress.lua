...
local MJPlayer = require("TZMJPlayer")
...
function GameProgress:ctor(room, roomId,unitCoin)
    ...
    self.huaList = {}
    self.buHuaFirstNum = 0
    for i = MJConst.Zi1, MJConst.Hua8 do
...
function GameProgress:broadcastMsg(key, pkg)
    self.room:broadcastMsg(key, pkg)
end
...
function GameProgress:broadcastTing(_seq, _myPos, _isPrevTing, showAni, toAll)
    local pkg = {}
    pkg.OperationSeq = _seq
    pkg.pos = _myPos
    pkg.isPrevTing = 1
    if not _isPrevTing then
        pkg.isPrevTing = 0
    end
    pkg.showAni = showAni or 1
    if toAll == 1 then
        self.room:broadcastMsg( "ting", pkg)
    else
        self.room:sendMsgToSeat(_myPos, "ting", pkg)
    end
    self.operHistory:addOperHistory("ting", pkg)
end

function GameProgress:syncTing()
    local playerList = self.playerList
    local seq = self:getOperationSeq()
    for pos = 1, 4 do 
        local player = playerList[pos]
        self:broadcastTing(seq, pos, player.isPrevTing, 0, 1)
    end
end

function GameProgress:broadcastTingAni(pos)
    local pkg = {}
    pkg.OperationSeq = self:getOperationSeq()
    pkg.pos = pos
    self.room:broadcastMsg("tingAni", pkg)
    self.operHistory:addOperHistory("tingAni", pkg)     
end
...
function GameProgress:sendMsgToSeat(pos, key, pkg)
    self.room:sendMsgToSeat(pos, key, pkg)
end
...
function GameProgress:sendMsgSetOperationTipCardsNotify(_pos, _data, tipNodes)
    ...
    if tipNodes then
        for k, v in pairs(tipNodes) do
            ...
            item.show = 1
            if player.isPrevTing == true then
                item.show = 0
            end
            item.huInfo = {}
...
function GameProgress:sendFlowerCnt(pos, uid, aniShow)
    if type(1) == type(pos) then
        local player = self.playerList[pos]
        local flowers = player:getFlowerMap() 
        local pkg = {}
        pkg.OperationSeq = self:getOperationSeq()
        pkg.pos = player:getDeskPos()
        pkg.flowerCard = ""
        pkg.cnt = table.size(player:getHuaList())
        pkg.showAni = 0 --aniShow or 0
        if table.size(flowers) > 0 then
            for card, cnt in pairs(flowers) do 
                local oldCard = MJConst.fromNow2OldCardByteMap[card]
                pkg.flowerCard = pkg.flowerCard..oldCard..":"..cnt..";"
            end
        end
        if uid == nil then
            self.room:broadcastMsg('showFlowerCnt', pkg)
        else
            self.room:sendMsgToUid(uid, 'showFlowerCnt', pkg)
        end
        self.operHistory:addOperHistory('showFlowerCnt', pkg)
    end

end

function GameProgress:sendAllFlowerCnt(uid)
    for pos = 1, 4 do
        self:sendFlowerCnt(pos, uid, 0)
    end
end

function GameProgress:sendMustPlayCards(pos)
    local pkg = {}
    pkg.seq = self:getOperationSeq()
    pkg.cards = self.playerList[pos]:getCanPlayCards()
    if self.playerList[pos].isTing == true then
        pkg.ting = 1
    else
        pkg.ting = 0    
    end
    self.room:sendMsgToSeat(pos, "mustPlayCards", pkg)
end

function GameProgress:calcuUpLimit()
    local upLimit = -1
    for str, _ in pairs(self.roomRuleMap) do 
        local strList = string.split(str, "_")
        local lastStr = strList[#strList]
        if lastStr == "WXZ" then
            break
        else
            if string.sub(lastStr, -1) == "b" then
                upLimit = tonumber(string.sub(lastStr, 1, string.len(lastStr) - 1))
            end
        end
    end
    return upLimit
...
function GameProgress:getUpLimit()
    if self.upLimit == nil then
        self.upLimit = self:calcuUpLimit()
    end
    return self.upLimit
end


function GameProgress:checkCalcKa()
    if self.calcKa == nil then
        if self.roomRuleMap["tz_playrule_kz"] == true then
            self.calcKa = 1
        end
        if self.roomRuleMap["tz_playrule_bskz"] == true then
            self.calcKa = 0
        end
    end
    return self.calcKa
...
function GameProgress:checkPengOnce()
    if self.pengOnce == nil  then
        if self.roomRuleMap["tz_extrarule1_sbp"] == true then
            self.pengOnce = 0
        end
        if self.roomRuleMap["tz_extrarule1_pf"] == true then
            self.pengOnce = 1
        end
    end
    return self.pengOnce
end

function GameProgress:checkSameKa()
    if self.sameKa == nil then
        if self.roomRuleMap["tz_extrarule2_tyws"] == true then
            self.sameKa = 1
        end
    end
    return self.sameKa
...
function GameProgress:checkPlayersHuaInHand()
    local bret = 1
    if self.buHuaFirstNum == 4 then
        bret = 0
    end
    return bret
end

function GameProgress:buHuProcess(player, sort)
    if player ~= nil then
        local huaByteCard = -1
        local newByteCard = -1
        local hasHua = 0
        if self.buHuaFirstNum < 4 then
            self.buHuaFirstNum = self.buHuaFirstNum + 1
        end
        local chandi = 0
        local newCardHua = nil 
        while(player:hasHua()) do
                huaByteCard = player:getHua()
                player:addHua(huaByteCard)
                newByteCard = self.mjWall:getFrontCard()

                if newByteCard == nil then
                    self.gameStateList[self.kGameEnd]:onEntry({isLiuJu = true})
                    return
                end
                if not player:delHandCard(huaByteCard) then
                    player:delNewCard(huaByteCard)
                    player:addNewCard(newByteCard)
                    if newCardHua == nil then
                        newCardHua = huaByteCard
                    end
                else
                    player:addHandCard(newByteCard)
                    self:sendBuHuaMesg(player, newByteCard, huaByteCard)
                    self:broadCastWallDataCountNotify()
                end
                if self.mjWall:getCanGetCount() == 3 then
                    LOG_DEBUG("player pos:"..player:getDeskPos().." buhua , then send three player card.")
                    chandi = 1
                    break
                end
                hasHua = 1
        ...
        if hasHua == 1 then
            if newCardHua ~= nil then
                self:sendBuHuaMesg(player, player:getNewCard(), newCardHua)
                self:broadCastWallDataCountNotify()
            end
            self:sendFlowerCnt(player:getDeskPos(), nil, 1)
        end
        if chandi == 1 then
            self.gameStateList[self.kGamePlaying]:gotoCycleGiveCard(player:getDeskPos())
        end
    end
end

function GameProgress:sendBuHuaMesg(player, newCard, huaCard)
    if player:checkZhiTing() == true then
        self:opBuHuaHandCardNotify(
            self:getOperationSeq(),
            player:getDeskPos(),
            {huaCard},
            0
        )
