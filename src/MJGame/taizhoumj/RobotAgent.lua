...
function CMD.enterGame(source, seatId)
    ...
    skynet.timeout(1, function()
        ...
        if roomid then
            ...
            --if ret.ret == true then
                --myRoomPos = ret.pos
            --end
            skynet.call(RACEMGR, "lua", "userSelectSeat", userid, myRoomPos)
        else
...
function CMD.relayToLobbyUser() end

function CMD.getMyIP(source)
end

function CMD.setWatcher(isWatcher) end
...
--        end)
    ...
    elseif data.status == 9999 then
        print("fangroom is disband, resetRobot:"..userid)
        skynet.call(ROBOTMGR, "lua", "resetRobot", userid)
    end
end
...
function REQUEST.showFlowerCnt(data) end

function REQUEST.notifyTip(data)
end

function REQUEST.recommendSuit(data)
end

function REQUEST.setAbandonedSuit(data)
end

function REQUEST.canPlayCards(data)
end
function REQUEST.notifyCheat()
end
function REQUEST.vipTotalScoreAH(data) end
function REQUEST.mustPlayCards(data) end
function REQUEST.playerDataInfo(data) end
function REQUEST.timerShow(data) end
function REQUEST.ting(data) end
...
function REQUEST.vipOver(data)
end

function REQUEST.vipErrorMsg(data)
end

function REQUEST.vipDisbandResult(data)
end

function REQUEST.vipDisbandVote(data)
end
...
function REQUEST.playerOperationReq(data)
    dump(data, "robot playerOperationReq")
    LOG_DEBUG("myroomPos:"..myRoomPos)
    local opSeq = data.OPerationSeq
    local onPlayerOperation = {}
    onPlayerOperation.OperationSeq = opSeq

    if myRoomPos == data.Pos and 0 ~= data.Op then
        ...
        elseif true == robotAI:checkOperation(MJConst.kOperLChi) then
            onPlayerOperation.operation = MJConst.kOperRChi
            onPlayerOperation.card_bytes = chiCard
        elseif true == robotAI:checkOperation(MJConst.kOperMChi) then
            onPlayerOperation.operation = MJConst.kOperRChi
            onPlayerOperation.card_bytes = chiCard
        elseif true == robotAI:checkOperation(MJConst.kOperRChi) then
            onPlayerOperation.operation = MJConst.kOperRChi
            onPlayerOperation.card_bytes = chiCard
        else
...
function REQUEST.requestVIPDisband(data) end
function REQUEST.vipRoomInfo(data) end
function REQUEST.vipOver(data) end
function REQUEST.vipErrorMsg(data) end
function REQUEST.vipDisbandResult(data) end
function REQUEST.vipDisbandVote(data) end
function REQUEST.notifyTip(data) end
function REQUEST.notifyUserCut(data) end
function REQUEST.vipTotalScore(data) end
function REQUEST.vipDisbandStatus(data) end
function REQUEST.addchicards(data) end
function REQUEST.dealHandCards(data) end
function REQUEST.voteBeforeGameInfo(data) end
function REQUEST.enterroomRes(data) end
function REQUEST.entergameNotify(data) end
function REQUEST.canSeatNotify(data) end
function REQUEST.checkHandData(data) end
function REQUEST.extraAwardCards(data) end
function printXXX(...)
    if arg then
        local printResult = ""
...
function isChiOp(_op)
    local isChi = false
    if MJConst.kOperLChi == _op or
            MJConst.kOperMChi == _op or
            MJConst.kOperRChi == _op then
        isChi = true
    end
    return isChi
end
...
skynet.start(function()
    ...
    math.randomseed(os.time())
    maxPlayCount = 9999 --math.random(4, 20)           --机器人随机玩4-20局
