...
function GamePlaying:checkSelfHu(turn)
    ...
    if player:canSelfHu() or player:canSelfHuQiDui() then
        ...
        local fCount = self:getFlowerCount(nil)
        count = fCount + count
        local ziMoHuaLimit = 0
        if self.gameProgress.roomRuleMap['sz_playrule_2m3c'] then
            ziMoHuaLimit = 2
        elseif self.gameProgress.roomRuleMap['sz_playrule_3m4c'] then
            ziMoHuaLimit = 2
        else
            ziMoHuaLimit = 2
        end
    ...
    if player then
        if false == player:hasNewCard() then
            ...
            if true == checkOp then
                ...
                elseif self.gameProgress.roomRuleMap.cz_playrule_7dui == true then
                    if player:canSelfHuQiDui() == true then
                        bHu = true
                    end
                end
...
function GamePlaying:getFlowerCount(byteCard)
    local sum = 0
    local windMke = self.countHuType:calcMPengCount()
    local windAKe  = self.countHuType:calcPengCountInHand()
    local windMG, normMG = self.countHuType:calcMGangCount()
    local windAG, normAG = self.countHuType:calcAGangCount()
    local windOtherAk = self.countHuType:calcHuOtherFeng(byteCard)
    sum =  windMke * 1 + windAKe * 2 + windAG * 4 + normAG * 2 + windMG * 3 + normMG * 1 + windOtherAk
    return sum
end
...
function GamePlaying:canHuOther(pos, turn, byteCard, isQiangGang)
    ...
    isQiangGang = isQiangGang or false
    if true == player:canHu(byteCard) or true == player:canHuQiDui(byteCard) then
        ...
        local count = #player.huaList
        dump(player.huaList, "hualist")
        --明杠1花，暗杠2花，风碰2花，风明杠3花，风暗杠4花
        local fCount = self:getFlowerCount(byteCard)
        count = fCount + count
        local chongHuaLimit = 0
        if self.gameProgress.roomRuleMap['sz_playrule_2m3c'] then
            chongHuaLimit = 3
        elseif self.gameProgress.roomRuleMap['sz_playrule_3m4c'] then
            chongHuaLimit = 3
    ...
    for i = 1, self.gameProgress.maxPlayerCount - 1 do  -- 仅允许除出牌人以外的人抢牌
        ...
        if not self.mjCancelMgr:isCancelOper(pos, MJConst.kOperHu, nil) and
            ...
            if true == player:canHu(byteCard)  then
                ...
                count = fCount + count
                --- 抢杠胡 ,仅需2花
                local otherFlowers = (_bGrabGang == true) and 2 or 3
                if count >= otherFlowers then
                    bHu = true
                else -- 小于4硬花，看有没有大胡
                    if self.countHuType:hasHuType(HuTypeConst.kHuType.kMengQing) then
                        bHu = true
            ...
            elseif self.gameProgress.roomRuleMap.cz_playrule_7dui == true then
                if player:canHuQiDui(byteCard) == true then
                    bHu = true
                end
            end
            if bHu then
...
function GamePlaying:doHu(pos, byteCard)
    ...
    -- zhuang lose, next pos zhuang.
    if table.keyof(posList, zhuangCopy) == nil then
        self.gameProgress.banker = self.gameProgress:nextPlayerPos(self.gameProgress.banker)
    end

    LOG_DEBUG("doHU Pos:"..pos.." card:"..byteCard)
    local cards = self.gameProgress:getNeedCardsAndBroadcast()
    local fans, type = self.gameProgress:calcExtraFans(zhuangCopy, posList, cards)
