...
function GameEnd:initCountHuType(args)
    self.winnerCountHuTypeList = {}
    for pos = 1, 4 do 
        self.countHuTypeList[pos].huTypeList = {}
    end
    ...
    local zhuangPos = self.gameProgress.banker
    local maxPlayers = self.gameProgress.maxPlayerCount
    for k, winPos in pairs(args.winnerPosList) do
        local relatedPos = (winPos - zhuangPos + maxPlayers) % maxPlayers + 1
...
function GameEnd:calculate(args)
    ...
    for k, countHuType in pairs(self.winnerCountHuTypeList) do
        ...
        if countHuType.isBaoPai then    -- 包牌
            ...
            details.caption = details.caption..";".."剃头"
...
function GameEnd:refreshTempData(args)
    ...
    --- refresh bar head cnt
    if barHeadType ~= "" then
        for pos, cnt in pairs(args.extraFans) do
            local num = cnt
            if barHeadType == "singleDragon" then
                if num > 0 then
                    num = 1
                end
            end
            self.gameProgress.tempData:updateValue(pos, "barHeadNum", num)
        end
    end
end
