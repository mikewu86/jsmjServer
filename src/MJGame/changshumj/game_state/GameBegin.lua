...
function GameBegin:onDice()
    ...
    --dump(self.gameProgress.roomRuleMap, "self.gameProgress.roomRuleMap")
    if self.gameProgress.roomRuleMap.cz_playrule_baozi then
        if dice1 == dice2 or mulNum == 4  then
            self.gameProgress.m_bDiZero = true
            self.gameProgress.m_bNextDiZero = false
        end

        if self.gameProgress.m_bNextDiZero == true then
            self.gameProgress.m_bNextDiZero = false
            self.gameProgress.m_bDiZero = true
        end
    else
...
function GameBegin:clearAllOpList()
    if type(self.playerList) == type({}) then 
        for k, v in pairs(self.playerList) do
            v.opHistoryList = {}
        end
    end
