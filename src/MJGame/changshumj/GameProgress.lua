...
local MJPlayer = require("ChShMJPlayer")
...
function GameProgress:getNeedCardsAndBroadcast()
   local bDlg = false
    local needCardNum = 0
    local zhuangPos = self.banker
    if self.roomRuleMap.cz_playrule_dlg == true then
        needCardNum = 1
        bDlg = true
    elseif self.roomRuleMap.cz_playrule_b4g == true then
        needCardNum = 4
    elseif self.roomRuleMap.cz_playrule_b6g == true then
        needCardNum = 6
    end
    local cards = {}
...
function GameProgress:calcExtraFans(zhuangPos, winPos, cards)
    local fans = {0, 0, 0, 0}
    local type = 0
    if #cards == 0 then
        return fans, type
    ...
    if self.roomRuleMap.cz_playrule_dlg == true then
        type = 1
        local type = calcSingleDragonBarType(cards[1])
        local score = 5
        if type == 0 then
            score = (cards[1] % MJConst.kMJPointNull) --* 10
        end
        for _, pos in pairs(winPos) do 
            fans[pos] = fans[pos] + score
        end
    else
        local maxCnt = self.maxPlayerCount or 4
        local result = calcMoveBarHead(cards, zhuangPos, maxCnt, nil)
        for pos = 1, maxCnt do 
            fans[pos] = fans[pos] + result[pos] --* 10
        end
    end
    return fans, type
end
