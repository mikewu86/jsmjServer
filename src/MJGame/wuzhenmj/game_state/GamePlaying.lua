...
function GamePlaying:checkHuCondition(limitHua, canTingNodes, turn)
    ...
    if player then
        ...
        for _, node in pairs(canTingNodes) do
            ...
            -- 出壳
            local _isChuQiao = false
            --if removeCard == player.newCard then
                if #node.huCardList > 27 then
                    _isChuQiao = true
                    node.isDiaoDa = 1
                    _node.isDiaoDa = 1
                end
                if self.gameProgress.roomRuleMap.wz_extrarule1_4bai 
                and baidaCount == 4 then
                    _isChuQiao = true
                    node.isDiaoDa = 1
                    _node.isDiaoDa = 1
                    --  LOG_DEBUG('nick: chuke2')
                end
            --end
            if tmpPlayer:delHandCard(removeCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    -- 检查胡牌花数
                    self.countHuType:setParams(turn, self.gameProgress, nil, nil, false, true, _isChuQiao)
...
function GamePlaying:checkMyOper(turn, checkOp)
    ...
    if player then
        if false == player:hasNewCard() then
            ...
            -- 出壳
            self.isChuQiao = false
            for k, v in pairs(canTingNodes) do
                if v.playCard == player.newCard then
                    if #v.huCardList > 27 then
                        self.isChuQiao = true
                    end
                end
            end
            if self.gameProgress.roomRuleMap.wz_extrarule1_4bai and baidaCount == 4 then
                self.isChuQiao = true
                -- LOG_DEBUG('nick: chuke1')
            end
    ...
    for i = 1, self.gameProgress.maxPlayerCount - 1 do  -- 仅允许除出牌人以外的人抢牌
        ...
        if not self.mjCancelMgr:isCancelOper(pos, MJConst.kOperHu, nil) then
            if true == player:canHu(byteCard, {self.gameProgress.baiDa}) then
                ...
                local bHu = false
                local baidaCount = player:hasBaiDa(self.gameProgress.baiDa)
                if (baidaCount > 0 and _bGrabGang)
                or  baidaCount == 0 then
                    bHu = true
                end
