...
--  KSMJPlayer
local KSMJPlayer = class("KSMJPlayer", BasePlayer)

function KSMJPlayer:ctor(maxCardCount, pos)
...
function KSMJPlayer:setConstHuaList(huaList)
    self.constHuaList = huaList
end

function KSMJPlayer:clear()
    self.super.clear(self)
end

function KSMJPlayer:reset()
    self:clear()
    self.huaList = {}
    self.huaGangList = {}
end
...
function KSMJPlayer:addHua(byteCard)
    table.insert(self.huaList, byteCard)
    self.justDoOper = MJConst.kOperBuHua
    table.insert(self.opHistoryList, self.justDoOper)
end

function KSMJPlayer:addHuaWithoutHis(byteCard)
    table.insert(self.huaList, byteCard)
end

function KSMJPlayer:clearOpList()
    self.opHistoryList = {}
end

-- 返回一张花牌
function KSMJPlayer:getHua()
    local cards = self:getHandCards()
    for k, v in pairs(cards) do
        if table.keyof(self.constHuaList, v) then
            return v
...
function KSMJPlayer:getHuaList()
    return self.huaList
end

function KSMJPlayer:getCardsForNums(_step)
...
-- 某种牌在明牌中数量去掉百搭
function KSMJPlayer:getSuitCountInHandWithBaida(suit, bIncludeNewCard,baida)
    local sum = 0
...
function KSMJPlayer:getCardNumInHua(_cardByte)
    local sum = 0
...
function KSMJPlayer:isHuaGang(_cardByte)
    -- local bRet = false
...
function KSMJPlayer:getHuaGangNum()
    local huaGangSum = #self.huaGangList
    return huaGangSum
end

-- 手牌中有没有百搭牌，有的话返回数量，没有返回0
function KSMJPlayer:hasBaiDa(_baiDa)
    local baiDaNum = 0
...
-- 获取所有可以听的牌,桐乡麻将不能胡七对
function KSMJPlayer:getCanTingCards(baiDaList)
...
-- 获取可以听以及可以出的牌
function KSMJPlayer:getTingNormalCards(baiDaList)
    local ret = {}
...
-- 生成一个新对象
function KSMJPlayer:clone()
    local player = KSMJPlayer.new(self.maxCardCount, self.myPos)
