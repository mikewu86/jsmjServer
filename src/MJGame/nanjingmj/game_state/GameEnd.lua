        ...
        if self.gameProgress:isVIPOver() then
            ...
            if self.gameProgress.isJYZ then
                for i = 1, #pkg.Score do
                    pkg.Score[i] = pkg.Score[i] - self.gameProgress.limitScore
                end
            end
...
function GameEnd:onEntry(args)
    ...
    if true == args.isLiuJu then
        ...
        for k = 1, 4 do
            local detail = {}
            detail.pos = i
            detail.describe = '杠分 '..gangScoreList[k]..';'..'罚分 '..faScoreList[k]..';'
            detail.caption = ''
            detail.pos = k
            table.insert(pkg.details, detail)
        end
        pkg.money = self.gameProgress:getRoundScore()
        self.operHistory:addOperHistory("gameResultNotifyNJMJ", pkg)
        self.gameProgress.room:broadcastMsg("gameResultNotifyNJMJ", pkg)
        ...
        -- 确定庄家
        local counterBanker = false  -- 连庄
        -- 上庄次数
        if self.gameProgress.banker ~= lastBanker then
            self.gameProgress.bankerCount = 1
        else
            self.gameProgress.bankerCount = 1 + self.gameProgress.bankerCount
        end
        --------------------------------
        ...
        -- 比下胡
        -- 确定是不是比下胡
        self.gameProgress.isBiXiaHu = false
        if table.keyof(args.winnerPosList, self.gameProgress.banker) ~= nil then
            self.gameProgress.isBiXiaHu = true -- 连庄
        else
            self.gameProgress.banker = self.gameProgress:nextPlayerPos(self.gameProgress.banker)
        end
        if self.gameProgress.isBiXiaHu == false then
            for _, countHuType in pairs(self.winnerCountHuTypeList) do
                if countHuType:hasHuType(HuTypeConst.kHuType.kGangKai) or 
                countHuType:hasHuType(HuTypeConst.kHuType.kXGangKai) or
                countHuType.isBaoPai or #args.winnerPosList > 1 or 
                self.gameProgress.hasHuaGang or self.gameProgress.hasFaFen then
                        self.gameProgress.isBiXiaHu = true
                end
            end
        
            if self.gameProgress.isJieZhuangBiXiaHu
               and #args.winnerPosList == 1 
               and args.winnerPosList[1] == self.gameProgress:nextPlayerPos(lastBanker) then
                self.gameProgress.isBiXiaHu = true
            end
        end
        --
...
function GameEnd:calculate(args)
    ...
    for k, countHuType in pairs(self.countHuTypeList) do
        if table.keyof(self.winnerCountHuTypeList, countHuType) ~= nil then
            ...
            details.describe = details.describe..'杠分 '..gangScoreList[k]..';'..'罚分 '..faScoreList[k]..';'
            table.insert(pkg.details, details)
        else
            local details = {}
            details.pos = k
            details.caption = ""
            details.describe = '杠分 '..gangScoreList[k]..';'..'罚分 '..faScoreList[k]..';'
            table.insert(pkg.details, details)
        end
    end
...
function GameEnd:calculateMoney(args)
    ...
    if self.gameProgress.isJYZ then
        ...
        if #self.winnerCountHuTypeList == 1 then
            ...
            if countHuType.isBaoPai  then  -- 包牌会和自摸重复，先判断包牌
                losePos = countHuType.baoPaiPos
                local player = self.gameProgress.playerList[countHuType.baoPaiPos]
                if self.gameProgress.limitScore > 0 and player:getLeftMoney() < totalMoney then
                    totalMoney = player:getLeftMoney()
                end
                money[countHuType.pos] = money[countHuType.pos] + totalMoney
            ...
            elseif args.isZiMo == false then
                ...
                for i = 1, self.gameProgress.maxPlayerCount do
                    if i ~= countHuType.pos then
                        if self.gameProgress.limitScore > 0 and self.gameProgress.playerList[i]:getLeftMoney() < perMoney then
                            money[i] = -self.gameProgress.playerList[i]:getLeftMoney()
                        else
                            money[i] = -perMoney
                        end
            ...
            local leftMoneys = {}
            for i = 1, self.gameProgress.maxPlayerCount do
                leftMoneys[i] = self.gameProgress.playerList[i]:getLeftMoney()
            end
            -- 从点炮的人开始，顺序给所有胡的人钱
            for i = 1, self.gameProgress.maxPlayerCount do
                local countHuType = self:getWinnerCountHuTypeByPos(from)
                if countHuType ~= nil then
                    ...
                    if self.gameProgress.limitScore > 0 and leftMoney < totalMoney then
                        totalMoney = leftMoney
                    end
