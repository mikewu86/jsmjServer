...
function GameBegin:onDice()
    ...
    if -1 == self.gameProgress.banker then
        ...
        if dice1 == deice2 then
            self.gameProgress.leopard = true
        end
    end
...
function GameBegin:dealHandCards()
    for k, v in pairs(self.playerList) do
        ...
        -- self.gameProgress.operHistory:addOperHistory(
        --     MJConst.kOperDealCards,
        --     {cards = v:getHandCards(), pos = i})
    end
    self.playerList[self.gameProgress.banker]:addNewCard(self.mjWall:getFrontCard())
    -- self.gameProgress.operHistory:addOperHistory(
    --         MJConst.kOperNewCard,
    --         {card = v:getNewCard(), pos = self.gameProgress.banker})
...
function GameBegin:onUserCutBack(_pos)
    ...
    pkg.handCards3 = playerList[3]:getCardsForNums(self.step)
    pkg.handCards4 = playerList[4]:getCardsForNums(self.step)
    --3.getFlowerCnt
