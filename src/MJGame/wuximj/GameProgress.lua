...
local MJPlayer = require("WXMJPlayer")
...
-- dump(self.playerList, "分配玩家")
    ...
    --self.curGameState = self.gameStateList[self.kTestCase]  -- 测试用

    self.doubleOpCount = 0 --- shuang suan
    self.leopard = false   --- baozi
...
function GameProgress:broadcastTing(_pos)
    local pkg = {}
    pkg.OperationSeq = self:getOperationSeq()
    pkg.pos = _pos
