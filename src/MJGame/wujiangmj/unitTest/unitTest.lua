...
gameProgress.banker = 2
gameProgress.roomRuleMap.cz_playrule_7dui = false
gameProgress.roomRuleMap.wj_playrule_baozi = true
...
local addDaCards = function(pos, card)
	local player = gameProgress.playerList[pos]
	if player then
		player:setDaFlag(true)
		player:addDaCard(cardMap[card])
	end
end
...
local fillHandCards = function(pos, cards)
	...
	if player then
		for _, v in pairs(cards) do
			...
			if cardMap[v] == player.daCard then
				addDaCards(pos, v)
			end
		end
	end
...
local setNewCard = function(pos, card)
	...
	if cardMap[card] == player.daCard then
		addDaCards(pos, card)
	end
end
		...
		if isHu then
			...
			for i = 1, #money do
				if money[i] ~= _result.money[i] then
					...
					dump(money, '实际分数')
					assert(false, _result.desc..'结算分数出错, 索引:'..runningIndex)
				end
			end
			...
			for i = 1, #huLists do
				...
				if #l ~= #r then
					assert(false, _result.desc..'胡牌类型有误, 索引:'..runningIndex)
				end
				table.sort(l)
				table.sort(r)
				for j = 1, #l do
					if l[j] ~= r[j] then
						dump(l, '期望牌型列表')
						dump(r, '实际牌型列表')
						assert(false, '胡牌类型出错, 索引:'..runningIndex)
					end
				end
			end
		end
...
for index, v in pairs(testcase) do
	...
	for pos, cards in pairs(v.handcards) do
		gameProgress.playerList[pos].daCard = cardMap[v.daCards[1]]
