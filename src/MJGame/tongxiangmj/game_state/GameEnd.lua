...
function GameEnd:onEntry(args)
    ...
    if true == args.isLiuJu then
        ...
        if self.gameProgress.roomRuleMap['tx_extrarule1_gangf'] then
            for k = 1, 4 do
                local detail = {}
...
function GameEnd:calculateMoney(args)
    ...
    for k, countHuType in pairs(self.winnerCountHuTypeList) do
        ...
        if countHuType.isBaoPai then  -- 抢杠 一家支付
            -- 16分封顶
            if self.gameProgress.roomRuleMap['tx_extrarule2_16fd'] then
                if totalMoney >= 16 then
                    totalMoney = 16
                end
            end
        ...
        if self.gameProgress.roomRuleMap['tx_extrarule1_gangf'] then
            local gangScoreList = self.gameProgress:getRoundGangScore()
            for i = 1, 4 do
                money[i] = money[i] + gangScoreList[i]
            end
        end
