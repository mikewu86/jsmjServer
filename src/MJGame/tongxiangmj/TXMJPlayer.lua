...
--  TXMJPlayer
local TXMJPlayer = class("TXMJPlayer", BasePlayer)

function TXMJPlayer:ctor(maxCardCount, pos)
...
function TXMJPlayer:setConstHuaList(huaList)
    self.constHuaList = huaList
end

function TXMJPlayer:clear()
    self.super.clear(self)
end

function TXMJPlayer:reset()
    self:clear()
    self.huaList = {}
    self.huaGangList = {}
end
...
function TXMJPlayer:addHua(byteCard)
    table.insert(self.huaList, byteCard)
    self.justDoOper = MJConst.kOperBuHua
    table.insert(self.opHistoryList, self.justDoOper)
end

function TXMJPlayer:addHuaWithoutHis(byteCard)
    table.insert(self.huaList, byteCard)
end

function TXMJPlayer:clearOpList()
    self.opHistoryList = {}
end

-- 返回一张花牌
function TXMJPlayer:getHua()
    local cards = self:getHandCards()
    for k, v in pairs(cards) do
        if table.keyof(self.constHuaList, v) then
            return v
...
function TXMJPlayer:getHuaList()
    return self.huaList
end

function TXMJPlayer:getCardsForNums(_step)
...
-- 某种牌在明牌中数量去掉百搭
function TXMJPlayer:getSuitCountInHandWithBaida(suit, bIncludeNewCard,baida)
    local sum = 0
...
function TXMJPlayer:getCardNumInHua(_cardByte)
    local sum = 0
...
-- function TXMJPlayer:isHuaGang(_cardByte)
    -- local bRet = false
...
function TXMJPlayer:getHuaGangNum()
    local huaGangSum = #self.huaGangList
    return huaGangSum
end

-- 手牌中有没有百搭牌，有的话返回数量，没有返回0
function TXMJPlayer:hasBaiDa(_baiDa, _bIncludeNewCard)
    local baiDaNum = 0
    local handCards = {}
...
-- 生成一个新对象
function TXMJPlayer:clone()
    local player = TXMJPlayer.new(self.maxCardCount, self.myPos)
...
-- 出壳检测
function TXMJPlayer:checkChuKe(_baiDa, _bIncludeNewCard, isQiDui)
    -- if self:hasNewCard() then
    --     return false
    -- end
    local countMap = nil
...
-- function TXMJPlayer:isRemainOneCard(_baiDa)
...
function TXMJPlayer:isRemainOneCard()
...
-- 获取所有可以听的牌,桐乡麻将不能胡七对
function TXMJPlayer:getCanTingCards(baiDaList)
...
-- 获取可以听以及可以出的牌
function TXMJPlayer:getTingNormalCards(baiDaList)
    local ret = {}
