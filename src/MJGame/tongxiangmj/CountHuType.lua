...
function CountHuType:calculate()
    ...
    -- 胡牌
    if not self:isTianHu() and not self:isDiHu() then
        -- 无花大吊捉冲不加胡分
    ...
    -- 有花出壳(飘花)
    -- dump(self.player:hasBaiDa(), "self.player:hasBaiDa()")
    -- dump(self.player:checkChuKe(self.gameProgress.baiDa), "self.player:checkChuKe(self.gameProgress.baiDa)")
    if self.player:hasBaiDa(self.gameProgress.baiDa) == 1 and self.player:checkChuKe(self.gameProgress.baiDa) then
        ...
        if riverCardsList[#riverCardsList] == self.gameProgress.baiDa then
            -- 如果有倒数第二张出牌,并且出的不是百搭
            if riverCardsList[#riverCardsList - 1] then
                if riverCardsList[#riverCardsList - 1] ~= self.gameProgress.baiDa then
                    huDetails.describe = huDetails.describe..'有花出壳飘花X2;'
                    hua = hua * 2
                    table.insert(self.huTypeList, HuTypeConst.kHuType.kPiaoHua)
                end
            else
                huDetails.describe = huDetails.describe..'有花出壳飘花X2;'
                hua = hua * 2
    ...
    if self.player:hasBaiDa(self.gameProgress.baiDa) == 1 and self.player:checkChuKe(self.gameProgress.baiDa) then
        local riverCardsList = self.riverCardList[self.pos]:getRiverCardsList()
        if #riverCardsList >= 2 then
            -- 如果河牌倒数第一,第二张是搭牌,就是三花飘两次
            if riverCardsList[#riverCardsList] == self.gameProgress.baiDa and riverCardsList[#riverCardsList - 1] == self.gameProgress.baiDa then
                huDetails.describe = huDetails.describe..'三花二飘X8;'
                hua = hua * 8
                table.insert(self.huTypeList, HuTypeConst.kHuType.kSanHuaErPiao)
            end
        end
    -- 三花飘一
    elseif self.player:hasBaiDa(self.gameProgress.baiDa) == 2 and self.player:checkChuKe(self.gameProgress.baiDa) then
        ...
        if #riverCardsList >= 1 then
            -- 如果河牌倒数第一张是搭牌,就是三花飘一次
            if riverCardsList[#riverCardsList] == self.gameProgress.baiDa then
                huDetails.describe = huDetails.describe..'三花一飘X4;'
                hua = hua * 4
                table.insert(self.huTypeList, HuTypeConst.kHuType.kSanHuaYiPiao)
            end
        end
    end
    ...
    if self:isGangKai() then
        -- 原手牌没有百搭
        if self.player:hasBaiDa(self.gameProgress.baiDa) == 0 then
            if self.player.newCard ~= self.gameProgress.baiDa then
                huDetails.describe = huDetails.describe..'无花杠开X2;'
                hua = hua * 2
                table.insert(self.huTypeList, HuTypeConst.kHuType.kGangKai)
            else
                -- 杠开补百搭胡牌,不翻倍
                huDetails.describe = huDetails.describe..'杠开摸花;'
                table.insert(self.huTypeList, HuTypeConst.kHuType.kGangKai)
            end
        else
            -- 有百搭在手
            huDetails.describe = huDetails.describe..'有花杠开;'
            table.insert(self.huTypeList, HuTypeConst.kHuType.kGangKai)
        end
    end
    ...
    if #riverCardsList >= 1 then
        if riverCardsList[#riverCardsList] == self.gameProgress.baiDa and self:isGangKai() then
            ...
            hua = hua * 4
            table.insert(self.huTypeList, HuTypeConst.kHuType.kPiaoGang)
        end
    end
    ...
    if self:isDaDiaoChe() then
        if self.player:hasBaiDa(self.gameProgress.baiDa) ~= 0 then
            huDetails.describe = huDetails.describe..'有花大吊X2;'
            hua = hua * 2
            table.insert(self.huTypeList, HuTypeConst.kHuType.kYouHuaDaDiao)
        else
            -- 没有花,摸到的新牌是花
            if self.player.newCard == self.gameProgress.baiDa then
                huDetails.describe = huDetails.describe..'有花大吊X2;'
                ...
                if self.isZiMo then
                    huDetails.describe = huDetails.describe..'无花大吊X4;'
                    hua = hua * 4
                    table.insert(self.huTypeList, HuTypeConst.kHuType.kWuHuaDaDiao)
                else
                    huDetails.describe = huDetails.describe..'无花大吊捉冲10分;'
                    hua = 10
                    table.insert(self.huTypeList, HuTypeConst.kHuType.kWuHuaDaDiao)
                end
            end
    ...
    if self.gameProgress.gangPiao[self.pos] > 0 then
        ...
        huDetails.describe = huDetails.describe..'杠飘X'..times..';'
        hua = hua * times
    ...
    -- 抢杠送杠*3
    if self.isQiangGang then
        huDetails.describe = huDetails.describe..'拉杠X3;'
    end
