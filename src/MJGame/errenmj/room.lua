...
function room:callSNGUpdateScore(scoreMap)
	if not sngmgr then
		sngmgr = skynet.uniqueservice("sngmgr")
	end
	return skynet.call(sngmgr, "lua", "updateScore", self.groupId, self.roundId, scoreMap, self.roomid)
end

function room:callSNGGameEnd()
	if not sngmgr then
		sngmgr = skynet.uniqueservice("sngmgr")
	end
	return skynet.call(sngmgr, "lua", "onGameEnd", self.groupId, self.roundId, self.roomid)
end

function room:callSNGBeforBegin()
	if not sngmgr then
		sngmgr = skynet.uniqueservice("sngmgr")
	end
	skynet.fork(function()
            return skynet.call(sngmgr, "lua", "beforGameBegin", self.groupId, self.roundId, self.roomid)
    end)
end

function room:updatePlayerScore(uid, score)
	local player = self.playingPlayers[uid]
	if nil ~= player then
		player:setSngScore(score)
	end
	self.GameProgress:broadCastMsgUpdatePlayerData()
end

function room:getPlayerScore(uid)
	local player = self.playingPlayers[uid]
	if nil ~= player then
		return player:getSngScore()
	else
		LOG_DEBUG("get uid:"..uid.." score fail.")
		return 0
	end
end
