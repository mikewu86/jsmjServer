...
function CMD.sendMsg(source, packageName, pkg)
    -- LOG_DEBUG('--robot sendMsg '..packageName)
    local f = CMD[packageName]
    if f then
	    pcall(f, pkg)
    else
        -- LOG_DEBUG('has no command named '..packageName)
        local f = assert(REQUEST[packageName], "game pkg: "..packageName.." not found")
        pcall(f, pkg)
    end
end
...
-- 请求进入房间，仅用在SNG游戏中
function CMD.enterSNGGame(groupid, roundid)
	-- LOG_DEBUG("recive user enterSNGGameReq:"..groupId)
	--此时处理为异步执行
    skynet.timeout(1, function()
        skynet.call(RACEMGR, "lua", "enterSNGGame", userid, skynet.self(), 0, groupId, roundid)
        if roomid then
            local ret = skynet.call(RACEMGR, "lua", "enterAllocatedRoom", userid, true)
            if ret.ret == true then
                myRoomPos = ret.pos
            end
        else
...
function CMD.enterAllocatedRoom(source, roomid, initMoney)
    -- LOG_DEBUG('robot enterAllocatedRoom roomid = '..roomid..' money = '..initMoney)
    local ret = skynet.call(RACEMGR, "lua", "enterAllocatedRoom",roomid, userid, true, initMoney)
    if ret.ret == true then
        myRoomPos = ret.pos
        return true
    end
...
function CMD.enterRoom(source)
    -- print("robotagent enterRoom call")
    if roomid then
        local ret = skynet.call(RACEMGR, "lua", "enterRoom", userid, nil, roomid, true)
        if ret.ret == true then
            myRoomPos = ret.pos
...
function CMD.dispatch(source, name, data)
    -- LOG_DEBUG(' robot agent dispath ..'..name)
...
-- 重新设置groupId, roomId
function CMD.changePlayingStatus(source, isPlaying, _roomid, _groupId)
    roomid = _roomid
    groupId = _groupId
    maxPlayCount = _maxPlayCount
end

function CMD.requestLeaveRoom(source)
...
function REQUEST.gameBeginNotify(data)
    robotAI:reset()
end

function REQUEST.gameEndNotify(data)
end

function REQUEST.sngKnockOut(data)
end

function REQUEST.sngOver(data)
end

function REQUEST.sngRankList(data)
end

function REQUEST.sngRound(data)
end
...
function REQUEST.ting(data)
end

function REQUEST.addchicards(data)
end

function REQUEST.setOperationTipCardsNotify(data)
    local opData = table.copy(data.Data)
    resetTipData()
    -- dump(opData, "setOperationTipCardsNotify")
...
function REQUEST.notifyGameLoadingBegin(data)
end

function REQUEST.notifyGameLoading(data)
end

function REQUEST.notifyGameLoadingEnd(data)
end
