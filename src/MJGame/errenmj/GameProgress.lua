...
local MJPlayer = require("MyPlayer")
...
function GameProgress:broadcastGameBegin()
    self.room:broadcastMsg('gameBeginNotify', {})
end

function GameProgress:broadcastGameEnd()
    self.room:broadcastMsg('gameEndNotify', {})
end
...
function GameProgress:broadcastTing(_seq, _myPos, _isPrevTing)
    local pkg = {}
    pkg.OperationSeq = _seq
    pkg.pos = _myPos
    pkg.isPrevTing = 1
...
function GameProgress:sendMsgSetOperationTipCardsNotify(_pos, _data, tingNodes)
    local pkg = {}
    pkg.OperationSeq = self:getOperationSeq()
    pkg.Data = {}
    for k, v in pairs(_data) do
        --dump(v, "pkg v")
        table.insert(pkg.Data, {
            op = k,
            cards = MJConst.transferNew2OldCardList(v)})
    end
    pkg.tingNodes = {}
    if tingNodes then
        for k, v in pairs(tingNodes) do
            table.insert(pkg.tingNodes, {
                playCard = MJConst.fromNow2OldCardByteMap[v.playCard],
                huCards = MJConst.transferNew2OldCardList(v.huCards),
                fans = v.fans
            })
        end
    end
    --dump(tingNodes, '-- tingNodes = ')
...
function GameProgress:broadCastMsgUpdatePlayerData()
    ...
    for pos = 1, self.maxPlayerCount do 
        ...
        if nil ~= player then
            ...
            --     pkg.Players[pos].money = player:get
            -- end
        end
    end
