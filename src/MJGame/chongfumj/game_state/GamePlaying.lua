        ...
        for _, node in pairs(canTingNodes) do
            ...
            if tmpPlayer:delHandCard(removeCard) then
                for _, canHudCard in pairs(node.huCardList) do
                    ...
                    self.countHuType:calculate()
                    local bHu = self:checkSelfHuOk(player)
...
-- 检查自己的操作
function GamePlaying:checkMyOper(turn, checkOp, pengGang)
    local player = self.playerList[turn]
    local ziMoHuaLimit = 0
    if player then
        ...
        if false == player:hasNewCard() then
            ...
            if true == checkOp then
                -- 胡牌
                if pengGang ~= true and (player:hasBaiDa(self.gameProgress.baiDa, true) > 2 or player:canSelfHu({self.gameProgress.baiDa}) )  then
                    self.countHuType:setParams(
                    turn, 
                    self.gameProgress, 
                    nil, 
    ...
    for i = 1, self.gameProgress.maxPlayerCount - 1 do  -- 仅允许除出牌人以外的人抢牌
        ...
        if not self.mjCancelMgr:isCancelOper(pos, MJConst.kOperHu, nil) then
            if true == player:canHu(byteCard, {self.gameProgress.baiDa}) then
                ...
                ---规则约束 抓冲 仅 -- 捉冲 or  地胡
                if self.countHuType:hasHuType(HuTypeConst.kHuType.kZhuoChong) == true or 
                    self.countHuType:hasHuType(HuTypeConst.kHuType.kDiHu) == true then
                    bHu = true
                end
...
function GamePlaying:onUserCutBack(_pos, _uid)
    ...
    self.gameProgress:broadcastPlayerClientNotify(
        operSeq,
        kDiceNumber, {self.gameProgress.diceNum[1], self.gameProgress.diceNum[2]})
end
...
function GamePlaying:clearCanDoMap(pos)
    if pos and self.playerCanDoMap[pos] then
        if table.keyof(self.playerCanDoMap[pos].operList, MJConst.kOperPlay) ~= nil then
            ...
            self.playerCanDoMap[pos].operList = {MJConst.kOperPlay}
        else
            self.playerCanDoMap[pos] = {}
        end
    else
...
function GamePlaying:checkSelfHuOk(player)
    local bHu = true
    local huaNum = player:hasBaiDa(self.gameProgress.baiDa, true)
    local huAll = player:checkChuKe(self.gameProgress.baiDa)
    local piaoHuaNum = self.gameProgress:getPiaoHuaStatus(player:getDeskPos())
    local gangKai = self:isGangKai(player)
    if huaNum > 0 and not huAll then
        bHu = false
    end

    if gangKai == true then
        bHu = true
    end
    if self.countHuType.daDiaoche == true then
        bHu = true
    end
    -- 第二飘除了飘的那个人,其余人只可以杠开 和大吊车,不能自摸
    if gangKai == true or self.countHuType.daDiaoche == true then
        for pos = 1, 4 do
            if turn ~= pos then
                local otherPlayer = self.gameProgress.playerList[pos]
                if not gangKai then
                    -- 有人飘了两次花
                    if piaoHuaNum > 1 then
                        bHu = false
                    else
                        if piaoHuaNum == 1 and not self.xiangGong[pos] and otherPlayer:checkChuKe(self.gameProgress.baiDa) then
                            if  huaNum == 2  or huaNum == 3 then
                                bHu = false
                            end
                        end
                    end
                ...
                -- -- 单吊飘
                -- if #tPlayer:getHandCardsCopy() == 1 and self.gameProgress:getPiaoHuaStatus(pos) >= 1 then
                --     if not gangKai then
                --         bHu = false
                --     end
                -- end
