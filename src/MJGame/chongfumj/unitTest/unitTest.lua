...
local addOutCards = function(pos, cards)
	local riverList = gameProgress.riverCardList[pos]
	if riverList then
		if cards then
			for _, card in pairs(cards) do 
				riverList:pushCard(cardMap[card])
			end
		end
	end
end

local checkResult = function(isZiMo,baida, poses, pos1, byteCard, args, _result)
	gameProgress.baiDa = cardMap[baida]
	local isHu = false
	if isZiMo then
		isHu = playingState:checkSelfHu(poses[1],cardMap[baida])
	else
		for _, pos in pairs(poses) do
			isHu = playingState:canHuOther(pos, pos1, byteCard,cardMap[baida])
		end
	end
	if isHu == _result.canHu then
		if isHu then
			...
			print( _result.desc..' test pass index:'..runningIndex)
		end
	else
		assert(false, _result.desc..' test error index:'..runningIndex)
	end
end
...
for index, v in pairs(testcase) do
	...
	for _, riverList in pairs(v.riverCards) do
		addOutCards(riverList.pos, riverList.cards)
	end
	--
	-- checkResult(args.isZiMo, args.baida, args.winnerPosList, 
	-- args.fangPaoPos, args.huCard, args, v.result)
end
fillHandCards(1, {"三万","三万","四万","四万","五万","五万","六筒","七筒","八筒","九筒","九筒",})
local player = gameProgress.playerList[1]
dump(player:getAllHandCards(), "handcards....")
dump(player:getCanPlayCards(), "canplayercards")