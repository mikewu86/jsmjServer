    ...
    var Sproto = {
        createNew: function(binsch) {
            ...
            function toword(stream) {
                var v = (stream[0] & 0xff) | (stream[1] & 0xff) << 8;
                return v;
            }
            ...
            function decode(args) {
                ...
                case SPROTO_TSTRING:
                    ...
                    for (var i = 0; i < args.length; i++) {
                        tmpArray[i] = args.value[i];
                        
                    }
                    if (tmpArray.length > 0) {
                        value = uintToString(tmpArray);
                    }
            ...
            sproto.dispatch = function(buf, req_cb, rsp_cb) {
                ...
                if (header.type) {
                    ...
                    if (header.session) {
                        return req_cb(proto.name, result, gen_response(proto.p[SPROTO_RESPONSE].name, header.session));
                    } else {
